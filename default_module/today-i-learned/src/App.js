...
function App() {
  ...
  useEffect(function() {
    async function getFacts() {
      ...
      else alert("There was a problem to get data");
...
function Header({ showForm, setShowForm }) {
  ...
  return (
    <header className="header">
      ...
      <button className="btn btn-open btn-large" onClick={()=> setShowForm((show) => !show)}>
...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      <input type="text" placeholder="Share a fact with the world..." name="" id="" value={text} onChange={(e) => setText(e.target.value)} disabled={isUploading} />
      <span id="maximumTextInput">{200 - textLength}</span>
      <input type="text" placeholder="Trustworthy source..." name="" id="" value={source} onChange={(e) => setSource(e.target.value)} disabled={isUploading} />
...
function Fact({ fact, setFacts }) {
  ...
  return (
    <li className="fact">
      <p>
          {fact.text}
          <a href={fact.source} target="_blank" className="source" rel="noreferrer">(Source)</a>
      </p>
